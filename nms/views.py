...
def ticket_pdf_view(request, ticket_id):
    ...
    try:
        fault_start = datetime.strptime(ticket.fault_start, '%Y-%m-%d %H:%M:%S')
        fault_end = datetime.strptime(ticket.fault_end, '%Y-%m-%d %H:%M:%S') if ticket.fault_end else None
        time_difference = fault_end - fault_start if fault_end else None
    except ValueError:
        fault_start, fault_end, time_difference = None, None, None

    # Calculate hours and minutes
    hours, minutes = (0, 0)
    if time_difference:
        hours, minutes = divmod(time_difference.total_seconds() // 60, 60)
    ...
    html_string = render(request, 'nms/ticket_pdf.html', context).content.decode('utf-8')

    # Convert HTML to PDF
    pdf_file = HTML(string=html_string).write_pdf()

    # Create and return PDF response
